...
namespace Drupal\extended_paragraph\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the 'entity_reference_localized_paragraphs paragraphs' widget.
 *
 * We hide add / remove buttons when translating to avoid accidental loss of
 * data because these actions effect all languages.
 *
 * @FieldWidget(
 *   id = "entity_reference_localized_paragraphs",
 *   label = @Translation("Paragraphs Localized"),
 *   description = @Translation("A asycnhronic paragraphs inline form widget."),
 *   field_types = {
 *     "entity_reference_revisions"
 *   }
 * )
 */
class LocalizedParagraphsWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    elseif (isset($widget_state['selected_bundle'])) {
      ...
      $paragraphs_entity = $entity_manager->getStorage($target_type)
        ->create(array(
    ...
    if ($paragraphs_entity) {
      ...
      elseif ($item_mode == 'preview') {
        ...
        $element['preview'] = entity_view($paragraphs_entity, 'preview', $paragraphs_entity->language()
          ->getId());
  ...
  protected function getCurrentLangcode(FormStateInterface $form_state, FieldItemListInterface $items) {
    return $form_state->get('langcode') ?: $items->getEntity()
      ->language()
      ->getId();
  }
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    if ($entity instanceof RevisionableInterface) {
      ...
      // property.
      elseif ($entity->getEntityType()
          ->hasKey('revision') && $form_state->getValue('revision')
      ) {
        $new_revision = TRUE;
      }
    }
  ...
  protected function cloneReferencedEntity(ContentEntityInterface $entity_to_clone, $langcode) {
    /** @var \Drupal\extended_paragraph\CloneReferencedEntityService $cloneEntity */
    $cloneEntity = \Drupal::service('extended_paragraph.clone_referenced_entity');
    return $cloneEntity->cloneEntity($entity_to_clone,$langcode);
  }
}
